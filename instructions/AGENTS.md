...
- detective (hypercharm/glm-5.3-flash, high thinking) → spawns research workers in parallel → each saves findings via its write_findings tool
...
- write_findings is NEVER delegated: NO agent may spawn a subagent (research or otherwise) to write findings, to look up the write_findings tool, or to "find the write_findings tool path" — the agent that produced the findings calls the tool ITSELF. `write_findings` is a plugin-injected tool; in subagent sessions the native call may fail with "Unknown tool: write_findings" because plugin tools are not propagated to subagent tool catalogs. **Always use the Code Mode wrapper directly** as the primary path: call `execute` with `return tools.write_findings({ path, body })` (plain single-quoted strings, \\ for backslashes, \n for newlines, no backticks — Code Mode rejects template literals with 'TaggedTemplateExpression'). The return value "WRITTEN: <path> (<n> bytes)" IS the write confirmation: return that path to the caller and do NOT read the file back to verify, and do NOT spawn an edit subagent to confirm or append to it.
...
- When delegating via the `subagent` tool, match the opening line to the target type and keep it VERBATIM, never appending role or capability declarations:
  ...
  - `agent: "research"` -> "You are a subagent. Search and read directly with your own tools; save findings with your write_findings tool. Do all searching yourself. Report findings concisely."
...
- Research: agnes-research/agnes-2.5-flash#research (LOW reasoning, fast)
