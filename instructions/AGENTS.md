...
- Findings-to-disk: ALL subagents (research, explore, edit) MUST write their full findings/report to a file under `.opencode-findings/` in the project root and return ONLY the file path plus a one-line summary. This survives compaction (findings on disk are not lost when history is summarised) and keeps the primary's context small (one line per spawn instead of a full report). The primary or `general` can read the file later if details are needed.
