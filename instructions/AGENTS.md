...
**All code modifications, file reads, searches, exploratory tasks, and shell**`instructions/AGENTS.md`**
````markdown
# Agent Delegation Rules
...
- Title tagging: when calling the Task tool, prefix the `description` parameter with the agent type tag - `[EDIT]` for edit, `[EXPLORE]` for explore, `[COORDINATE]` for general - so subsession titles are immediately identifiable in the session tree.
