...
- Title tagging: when calling the `subagent` tool, prefix the `description` parameter with the agent type tag - `[??Edit]` for edit, `[??Coordinate]` for coordinator, `[??Research]` for research, `[???????Detective]` for detective, `[??Explore]` for explore, `[??General]` for general � so subsession titles are immediately identifiable in the session tree. Do NOT tag the primary session itself.
...
- Findings writes are ONE write_findings call: path under `.opencode-findings/`, body = full markdown. Aliases accepted (file/content/text), tolerant parsing (case, quotes, base64 variants). If the tool is not in your catalog, ONE execute call with plain string params - no multi-step shell/base64/temp-file detours. The agent that produced the findings calls it ITSELF - never spawn a subagent to write findings or look up the tool. The WRITTEN response IS the confirmation: return that path plus a one-line summary and do NOT read the file back to verify, and do NOT spawn an edit subagent to confirm or append to it.
