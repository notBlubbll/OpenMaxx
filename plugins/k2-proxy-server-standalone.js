// k2-proxy-server-standalone.js — stdlib-only reverse proxy fixing IFM K2-Horizon
...
// assistant message missing thinking-ish fields.
//
// Standalone script — no OpenCode plugin interface. Import via k2-reasoning-proxy.js.
...
function maybePatchBody(raw, contentType) {
  ...
  let parsed;
  try { parsed = JSON.parse(raw.toString("utf8")); } catch { return { body: raw, parsed: null }; }
  ...
  for (const m of parsed.messages) {
    ...
    let mutated = false;
    for (const f of THINK_FIELDS) { if (f in m) { delete m[f]; mutated = true; } }
    if (typeof m.reasoning !== "string") { m.reasoning = ""; mutated = true; }
...
const server = createServer(async (req, res) => {
  try {
    ...
    const fingerprint = fingerprintPayload(JSON.parse(raw?.toString("utf8") ?? "{}"));
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      if (isIfm) { headers["authorization"] = `Bearer ${IFM_KEYS[usedIdx]}`; init.headers = headers; }
      try { upstream = await fetch(UPSTREAM + path, init); } catch (err) { lastErr = err; if (!isIfm) break; keyHealth[usedIdx] = { healthy: false, lastError: Date.now() }; usedIdx = pickKey(undefined); continue; }
      const retryable = upstream.status === 429 || upstream.status === 401 || upstream.status === 403;
      if (retryable && attempt + 1 < maxAttempts) { await upstream.body?.cancel?.(); keyHealth[usedIdx] = { healthy: false, lastError: Date.now() }; usedIdx = pickKey(undefined); if (cachedSession) cachedSession.tokenIndex = usedIdx; continue; }
      break;
    }
    if (upstream === null) { if (!res.headersSent) res.writeHead(502, { "content-type": "text/plain" }); res.end(`Proxy error: ${lastErr && lastErr.message ? lastErr.message : String(lastErr)}`); return; }
    if (fingerprint != null) { if (!cachedSession) { trackConversationSession(fingerprint, { tokenIndex: usedIdx, requestCount: 1 }); } else { cachedSession.requestCount++; cachedSession.tokenIndex = usedIdx; trackConversationSession(fingerprint, cachedSession); } }
    ...
    try { res.end(`Proxy error: ${err && err.message ? err.message : String(err)}`); } catch {}
  }
});
