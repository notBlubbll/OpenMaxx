// k2-reasoning-proxy.js — OpenCode V2 plugin that autostarts k2-proxy-server.js.
  ...
  async setup(ctx) {
    ...
    async function ensureRunning() {
      try {
        ...
        child.unref();
        await ctx.app?.log?.({ body: { service: "k2-proxy", level: "info", message: `k2-proxy starting (http://${HOST}:${PORT} -> https://api.ifm.ai)` } }).catch(() => {});
        setImmediate(async () => {
          try {
            await new Promise((r) => setTimeout(r, 1500));
            if (!(await isUp())) {
              const hint = `health check still failing after spawn — possible EADDRINUSE: stale 127.0.0.1:8089 listener; run 'netstat -ano | findstr :8089' and restart opencode`;
              logSpawnError(hint);
            }
          } catch {}
        });
      } catch (err) {
        await ctx.app?.log?.({ body: { service: "k2-proxy", level: "error", message: `ensureRunning failed: ${err && err.message ? err.message : String(err)}` } }).catch(() => {});
      }
    }
