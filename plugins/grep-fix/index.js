...
// are treated as one literal glob.
//
// This version:
...
export default {
  id: "grep-fix",
  async setup(ctx) {
    // 1) Before grep runs, validate pattern and normalize include.
    const hook = async (event) => {
      if (event.tool !== "grep") return
      const input = event.input || {}

      // Validate regex pattern for balanced delimiters
      ...
      if (typeof pat === "string" && pat.length > 0) {
        ...
        if (depth !== 0 || inClass) {
          // Try to fix common issues: strip trailing garbage after unbalanced parens
          // Report a clear error so the agent can fix the pattern
          event.error = { message:
            "Regex has unbalanced parentheses (depth=" + depth + "). " +
            "Simplify the pattern: use fewer groups, avoid nested alternations. " +
            "Try splitting into multiple simpler grep calls instead of one complex regex.\n" +
            "Original pattern: " + pat
          }
          event.status = "error"
          return
        }
