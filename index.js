...
const MIND_BIN = "C:\\Users\\User\\.bun\\bin\\bun.exe";
const MIND_ENTRY = "C:\\Users\\User\\Documents\\mind\\src\\mind.ts";
...
function runMindCommand(args) {
  const baseOptions = { encoding: "utf-8", stdio: ["ignore", "pipe", "pipe"], timeout: 5000, windowsHide: true };
  let result = spawnSync(MIND_BIN, [MIND_ENTRY].concat(args), baseOptions);
  if (result.status === 0) return { ok: true, stdout: String(result.stdout ?? ""), stderr: String(result.stderr ?? "") };
  result = spawnSync(FALLBACK_MIND_BIN, [MIND_ENTRY].concat(args), baseOptions);
